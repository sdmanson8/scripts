...
$tweaks = @(
   ...
   # Services - WindowsSearch #
    ...
    "DeleteTempFiles",
    #"CleanWinSXS", 
    "RemoveUnneededComponents", ## We remove the unconnected components to manage the InputLag.
    "EnableWindowsSearch",         
...
Function DisableExtraServices {
    ...
    foreach ($service in $services) {
        if (Get-Service $service -ErrorAction SilentlyContinue)
        {
            ...
            Write-Host " We ignore $service (The service doesn't exist)"
        }
    }
}
...
Function RemoveUnneededComponents {
    $components = @(
    #'Printing-PrintToPDFServices-Features',  # This service delete the Print to PDF 
    ...
    foreach ($component in $components) {
        Write-Host "Uninstalling : $component"
...
Function DisableGPDWinServices {
    Write-Host "Deactivation of external services (GPD Win)..."
 
    $services = @(
        "Spooler"                                   # This option can disable the printer - To fix it, search in services.msc and reactivate the Spooler!
        "TabletInputService"                        # 
    )
    ...
    foreach ($service in $services) {
        if (Get-Service $service -ErrorAction SilentlyContinue)
        {
            Write-Host "Desactivation and stopping $service process"
            Stop-Service -Name $service
            Get-Service -Name $service | Set-Service -StartupType Disabled
        } else {
...
Function WaitForKey {
    Write-Host
    Write-Host "Terminus !"
    Read-Host "Press Enter to instantly reboot your PC and finish the script, Enjoy !"
}
